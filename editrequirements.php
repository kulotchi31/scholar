...
if(!empty($update_image)){
  if($update_image_size > 2000000){
     ...
     $image_update_query = mysqli_query($con, "UPDATE `requirements_tbl` SET `tax_exemption`='$update_image' WHERE requirements_id = '".$_GET['id']."'") or die('query failed');
...
<div class="card" style="width: 18rem; margin-right: 30px; margin-top: 25px;" >
  ...
  <?php
        ...
        $query = "SELECT * FROM requirements_tbl WHERE requirements_id = '".$_GET['id']."' ";
        if($result = mysqli_query($con, $query)) {
        $row = mysqli_fetch_assoc($result);
          echo '<img class="img1" src="upload/'.$row['tax_exemption'].'" width="250" height="250" style=" margin-bottom: 25px;" >';
        } else {
          die("Error with the query in the database");
        }
      ?>
<strong><label>Tax</label></strong>
...
<div class="card" style="width: 18rem; margin-right: 30px; margin-top: 25px;" >
  ...
  <?php
        ...
        if($result = mysqli_query($con, $query)) {
        $row = mysqli_fetch_assoc($result);
          echo '<img class="img1" src="upload/'.$row['COR'].'" width="250" height="250" style=" margin-bottom: 25px;" >';
        } else {
          die("Error with the query in the database");
        }
      ?>
<strong><label>COR</label></strong>
...
<div class="card" style="width: 18rem; margin-right: 30px; margin-top: 25px;" >
  ...
  <?php
        ...
        if($result = mysqli_query($con, $query)) {
        $row = mysqli_fetch_assoc($result);
          echo '<img class="img1" src="upload/'.$row['COG'].'" width="250" height="250" style=" margin-bottom: 25px;" >';
        } else {
          die("Error with the query in the database");
        }
      ?>
<strong><label>COG</label></strong>
...
<!-- <input type="submit" value="update profile" name="update_profile" class="btn">
         ...
         <div class="button">
          ...
          <a href="requirements.php" class="btn btn-danger">Cancel</a>
        </div>
      </form>
